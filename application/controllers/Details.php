	...
	{ 
		...
		if(isset($this->session->userdata['logged_in'])) {
			if($this->session->userdata['logged_in']['acct_type'] == "admin") {
				...
				// $this->session->unset_userdata('filtered_value', $sess_array);
				 
			 
				 $this->load->view("doctorToPatientList_view");
				 //$this->load->view("admintasklist_temp_vi
               ...
               // $this->session->unset_userdata('filtered_value', $sess_array);
				//$data['tasks'] = $this->admintaskmngt_model->getTaskList();
	...
	function blockDoc()
	{
		if(isset($this->session->userdata['logged_in'])) {
			if($this->session->userdata['logged_in']['acct_type'] == "admin") {
				 ...
				 $updatedata = array('id' => $this->input->post('id'),
									  'Status' => 2,
								      'last_login' => $date
							 );
	...
	function updatePatient()
	{
		if(isset($this->session->userdata['logged_in'])) {
			if($this->session->userdata['logged_in']['acct_type'] == "admin") {
				 ...
				 $updatetdata = array('id' => $this->input->post('docid'),
	...
	function prescribeToPatient()
	{
		...
		if(isset($this->session->userdata['logged_in'])) {
			if($this->session->userdata['logged_in']['acct_type'] == "doctor") {
				 ...
				 $prescribedata = array('bookingId' => $this->input->post('bookId'),
	...
	function bookDoctor()
	{
		...
		if(isset($this->session->userdata['logged_in'])) {
			if($this->session->userdata['logged_in']['acct_type'] == "patient") {
				 $this->session->userdata['logged_in']['acct_type'];
				 $date = date("Y-m-d");
				 $bookingdata = array('docId' => $this->input->post('docid'),
