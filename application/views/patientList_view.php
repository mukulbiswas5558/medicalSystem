...
$('#saveEmpForm').submit('click',function(){
    ...
    var docpassword = $('#password').val();
		...
		$.ajax({
			type : "POST",
			url  : "<?php echo base_url(); ?>Details/addPatient",
			dataType : "JSON",
			data : {Title:docTitle, Name:docName, UserName:docUserName, State:docState, Phone1:docPhone1, Location:docLocation, password:docpassword},
    ...
    var docpassword = $('#docpassword').val();			
		...
		$.ajax({
			type : "POST",
			url  : "<?php echo base_url(); ?>Details/updatePatient",
			dataType : "JSON",
  ...
  $('#listRecords').on('click','.deleteRecord',function(){
	...
	// delete emp record
	 $('#blockDoctorForm').on('submit',function(e){
		...
		console.log(id);
		console.log('call in delet submit');
		$.ajax({
			type : "POST",
